...
十一项原生支持分别绑定到十一个不同方向键；它们都是独立 `new_signal`，不能互相算作 `reinforces`。当前证据只增加早期信号，领域地图保持不变。
