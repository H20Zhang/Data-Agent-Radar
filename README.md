...
<a id="entry-2609-04641"></a>
<details><summary>2026-09-08 · DevRev NL2SQL · Data Discovery & Grounding <!-- timefirst:area=data-discovery-grounding --> — 把嵌套模式可见性与错误恢复变成显式控制面 <!-- timefirst:delta=feedback-guided-nested-schema-grounding --></summary>
...
**证据。** 在 DevRev 的匹配消融中，富元数据被拿掉后整体下降 15.0pp（`metadata ablation loses 15 points`）；关闭反馈驱动的模式扩展下降 8.5pp，把类型化错误路由换成通用重试下降 13.2pp，平均修复轮数从 1.4 增至 3.2。<!-- timefirst:evidence=nested-schema-control-ablation~metadata-ablation-loses-15-points -->

**限制。** 该论文的 91.7% 是一整套系统在单个 LLM 辅助生成的 Snowflake benchmark 上的结果；关键混杂是 `generated benchmark interface co-designed`：基线看到的模式信息与框架并未完全配平，最终分数还包含 GPT-5.2 语义判断，承诺公开的数据集目前也尚未释放。<!-- timefirst:caveat=interface-and-benchmark-alignment~generated-benchmark-interface-co-designed -->
...
<details><summary>2026-09-02 · Agentic Data Cracking · Learning & Reuse <!-- timefirst:area=learning-reuse --> — 把已读取文档逐步物化为跨查询结构 <!-- timefirst:delta=workload-adaptive-structured-reuse --></summary>

**问题。** 一次 data-reasoning query 已经为打开长文档支付 prefill 后，系统能否把其中部分证据结构持久化，让后续目标不同但访问相同文档的查询少付一次原文读取成本？完整标题：*Token-Efficient Data Reasoning Agents via Adaptive Structuring of Unstructured Data*。<!-- timefirst:question=cross-query-structured-reuse -->
...
**链接。** [论文](https://arxiv.org/abs/2608.31082) · [中文深读](papers/2608.31082.md) · [英文深读](papers/2608.31082.en.md)

</details>

<a id="entry-2608-25210"></a>
<details><summary>2026-08-28 · BLIP · Verification & Recovery <!-- timefirst:area=verification-recovery --> — 把 provenance 变成可执行的答案复现证据 <!-- timefirst:delta=verifiable-answer-provenance --></summary>

**问题。** 对 LLM 数据处理结果，能否把“看起来相关的 citation”升级为一个可重新执行的输入子集，使同一模型在只看到该子集时仍复现相同或等价答案？完整标题：*Bolt-on, Verifiable Provenance for LLM-Powered Data Processing*。<!-- timefirst:question=answer-reproduction-provenance -->

**证据。** 论文在 `seven datasets over thirty percent` 的比较中报告：BLIP 的 provenance 满足答案复现约束，并在 provenance size 可比时比最佳 baseline 的 provenance accuracy 高超过 30%，同时总推理成本与原始完整输入查询处于同一量级。<!-- timefirst:evidence=answer-preservation-guarantee~seven-datasets-over-thirty-percent -->

**限制。** 最关键的边界是 `single invocation not business truth`：它验证的是当前模型答案的可复现性；完整输入答案若错误或歧义，provenance 仍可能稳定复现错误，而且当前定义没有覆盖多工具、多步骤 Agent workflow。<!-- timefirst:caveat=reproducibility-truth-boundary~single-invocation-not-business-truth -->

**地图。** `early_signal`；进入验证与恢复，把 source relevance、answer-reproduction provenance 与 answer / business correctness 分开；单篇证据不修改稳定地图。

**链接。** [论文](https://arxiv.org/abs/2608.25210) · [中文深读](papers/2608.25210.md) · [英文深读](papers/2608.25210.en.md)
...
**链接。** [论文](https://arxiv.org/abs/2608.22651) · [中文深读](papers/2608.22651.md) · [英文深读](papers/2608.22651.en.md)
...
| **数据库交互应该加多少结构？** | APEX-SQL → ReAct-SQL → Semantic-Layer-Mediated Agent | 先区分 grounding probe policy 与 query representation：主动探查可以解决值域不确定性，typed IR 可以解决组合错误；更重的语义层还承担业务定义与跨数据库治理。 |
| **跨查询重复读取同一批文档时，结构应何时累积？** | Agentic Data Cracking → AgentSM | 前者把 workload 已经支付的 document reads 物化为共享 data substrate，后者复用 database-specific execution knowledge；需要分别验证 locality / invalidation 与 schema / workload drift。 |
| **科学数据如何从发现走到可辩护主张？** | Scientific Data Skills → MAESTRO → Brain Researcher | 数据集语义、计算 campaign 与主张审查必须保留各自的来源和失败边界；可复现执行仍不等于科学真实。 |
| **产物与证据何时足以获准发布？** | Bolo → Artic → KDAF → BLIP → TRACE | runtime、artifact dataflow、来源可回放性、答案复现 provenance 与 write policy 是不同 gate；可复现当前模型答案仍不能替代业务真实性。 |
