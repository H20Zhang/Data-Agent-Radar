...
After a versioned switch to native Radar-acceptance timing, native directions use the family gates: `new_signal` has exactly one `early_signal` support and `prior=none`; `reinforced` has at least two distinct in-window native supports carrying the same exact direction key plus `prior=field-map`; `revised | splits | retires` have canonical native support plus the same prior Field Map evidence; and `no_material_change` has `supports=none` and `prior=none`.

The current legacy-publication adapter is deliberately narrower. Every direction has `state=new_signal`, exactly one `legacy_unknown` support whose `direction_keys` contains the block key, support `map_delta=none | early_signal`, and `prior=none`. It never emits `reinforced`, `revised`, `splits`, `retires`, or `no_material_change`, and never edits the durable map. Polaris stays under `dynamic-orchestration-package` and Business Truth under `business-truth-safety-contract`; neither reinforces the other. The visible time basis remains `legacy_publication_date`, explicitly **not Radar acceptance**.
